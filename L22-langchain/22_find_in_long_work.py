...
import spacy
from sample_text_1 import SAMPLE_TEXT
...
long_text = st.text_area(label="Paste your long text here.", height=300, value=SAMPLE_TEXT)
...
if button:
    if long_text:
        ...
        llm = ChatOllama(model='llama3.1')  # Specify your model here
...
# """
        messages = [
            ('system','You are a helpful assistant. Please answer the following question based on the below text:'),
        ...
        #     ]
        # )
        prompt_template = ChatPromptTemplate(messages=messages, input_variables=["text", "question"])
        ...
        # chain = RunnableSequence.from([promptTemplate, new ChatOllama({model='llama3.1'})]);
